...
type ResolvedShareQuery = ReturnType<typeof useLocalQuery<ResolvedShare | null>>;

const ResolvedShareContext = React.createContext<ResolvedShareQuery | null>(null);

export function VerifyResolvedShareProvider({
  children,
}: {
  ...
  return (
    <ResolvedShareContext.Provider value={query}>
      {children}
    </ResolvedShareContext.Provider>
  );
}

function useResolvedShare() {
  const query = React.useContext(ResolvedShareContext);
  if (!query) throw new Error("useResolvedShare must be used within VerifyResolvedShareProvider");
  return query;
}

export function VerifyPacketStatus() {
  const query = useResolvedShare();
  if (!query.data) return null;

  const state = packetStateOf(query.data.payload.packet);

  return (
    <StatusBadge tone={state === "active" ? "verified" : "warning"}>
      {state === "active" ? "Active link" : state === "revoked" ? "Revoked" : "Expired"}
    </StatusBadge>
  );
}

export function VerifyPacketContent() {
  const query = useResolvedShare();
  ...
  return <VerifyContent resolved={query.data} />;
}
