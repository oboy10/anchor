...
function messageType(message: AnchorPresentationBundle["messages"][number]) {
  return message.body.payload.find((entry) => entry.k === "a:type")?.v;
}
...
export async function issueAnchorMessage(
  ...
  if (input.payload) ensurePayloadHasType(input.payload, input.type);
  const signedMessage =
    input.signedMessage ??
    (input.payload
      ? await adapter.signTypedMessage({
          issuer,
          subject,
          type: input.type,
      ...
      : undefined);
  if (!signedMessage) {
    throw new Error("Issuance requires a signed message.");
  }
  if (signedMessage.body.from !== issuer.fingerprint) {
    throw new Error("Signed message issuer does not match issuance intent.");
  }
  if (signedMessage.body.to !== subject.fingerprint) {
    throw new Error("Signed message subject does not match issuance intent.");
  }
  if (messageType(signedMessage) !== input.type) {
    throw new Error("Signed message type does not match issuance intent.");
  }

  const verification = await adapter.verifySignedMessage(signedMessage);
  if (!verification.valid) {
    throw new Error(`Signed message failed verification: ${verification.flags.join("; ")}`);
  }
