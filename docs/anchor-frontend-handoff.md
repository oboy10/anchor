...
| `POST` | `/api/anchor/issuance` | Submit issuer intent; backend validates directory state and delegates signing to the protocol adapter. |
...
`lib/anchor/protocol-adapter.ts` currently exports an unsafe demo adapter so routes and frontend work can proceed. Agent B should replace it with the real protocol package without changing the DTOs or route responses.

Frontend code should treat warnings containing `unsafe demo protocol adapter` as development-only and not product copy.
